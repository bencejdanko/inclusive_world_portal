"""Form detail view for taking forms."""
import logging
...
class FormDetail(View):
    """View for displaying and submitting form responses."""

    @survey_available
    def get(self, request, *args, **kwargs):
        """Handle GET request to display survey."""
        ...
        asset_context = {
            # If any of the widgets of the current form has a "date" class, flatpickr will be loaded
    ...
    def post(self, request, *args, **kwargs):
        """Handle POST request to submit survey response."""
        survey = kwargs.get("survey")
        if survey.need_logged_user and not request.user.is_authenticated:
            return redirect(f"{settings.LOGIN_URL}?next={request.path}")
